...
, openxr-loader
, embree, gmp, libharu
, upbge-addons, upbge-logicnodes, pulseaudio
}:
...
stdenv.mkDerivation rec {
  ...
  src = fetchFromGitHub {
    ...
    repo = "upbge";
    rev = "v${version}";
    hash = "sha256-Xu5csZ63a5ipHzwzPgQ/Xs+GfoCWpkzVwmt/kA5Gw54=";
  ...
  pythonPath = with python310Packages; [ numpy requests upbge-uplogic toml ];

  prePatch = ''
    mkdir -p scripts/addons/bge_netlogic
    cp -a ${upbge-addons}/* scripts/addons
    find scripts/addons -type d -exec chmod 755 {} \;
    find scripts/addons -type f -exec chmod 644 {} \;
    cp -a ${upbge-logicnodes}/* scripts/addons/bge_netlogic
    find scripts/addons/bge_netlogic -type d -exec chmod 755 {} \;
    find scripts/addons/bge_netlogic -type f -exec chmod 644 {} \;
  '';
  ...
  '' + ''
    mv $out/share/upbge/${lib.versions.majorMinor blenderVersion}/python{,-ext}
  ...
  meta = with lib; {
    description = "open-source, 3D game engine forked from the old Blender Game Engine and deployed with Blender itself";
